...
These functions are specifically designed to work with HCP-D data downloaded around Aug 2020 on CHPC at Washington University in St. Louis.
N.B. Here I only process Resting-state (rfMRI_Rest*_AP, rfMRI_Rest*_PA) not task
"""
import glob
import os
import re
...
def convert_hcpd_to_bids_single_subject(in_dir, out_dir, sub_ent):
    ...
    .. code-block::
        ...
        sub-<sub_id>
                └── MNINonLinear
                    ...
                    │   │   ├── *_<TASK_ID><RUN_ID>_<DIR_ID>_Atlas.dtseries.nii # to make it comparable to other dataset I'm using the one without MSMAll.
    ...
    for base_task_name in task_names:
        ...
        bold_cifti_orig = os.path.join(
            task_dir_orig,
            f"{base_task_name}_Atlas.dtseries.nii",
        )
